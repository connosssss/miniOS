...
namespace pmm {
    void init(uint32_t multiboot_info_addr) {
        ...
        // make bitmap start past loaded multiboot modules if present
        if (mbi->mods_count > 0) {

            auto* mods = reinterpret_cast<multiboot_module_t*>(mbi->mods_addr);

            for (uint32_t m = 0; m < mbi->mods_count; m++) {
                uint32_t mod_end_aligned = align_up(mods[m].mod_end, FRAME_SIZE);

                if (mod_end_aligned > bitmap_start_addr) {
                    bitmap_start_addr = mod_end_aligned;
                }
            }
        }
        ...
        if (mbi->mods_count > 0) {
            ...
            for (uint32_t m = 0; m < mbi->mods_count; m++) {
                for (uint32_t a = mods[m].mod_start; a < mods[m].mod_end; a += FRAME_SIZE) {
                    uint32_t frame = a / FRAME_SIZE;
                    if (frame < max_frames) set_used(frame);
